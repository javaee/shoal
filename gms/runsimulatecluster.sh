...
./rungmsdemo.sh instance05 ${GROUPNAME} CORE 0 CONFIG ${TRANSPORT} -ts 9361 -te 9390 -ma ${MULTICASTADDRESS} -mp ${MULTICASTPORT} > ${LOGS_DIR}/instance05.log 2>&1 &
./rungmsdemo.sh instance06 ${GROUPNAME} CORE 0 CONFIG ${TRANSPORT} -ts 9430 -te 9460 -ma ${MULTICASTADDRESS} -mp ${MULTICASTPORT} > ${LOGS_DIR}/instance06.log 2>&1 &
./rungmsdemo.sh instance07 ${GROUPNAME} CORE 0 CONFIG ${TRANSPORT} -ts 9461 -te 9490 -ma ${MULTICASTADDRESS} -mp ${MULTICASTPORT} > ${LOGS_DIR}/instance07.log 2>&1 &
./rungmsdemo.sh instance08 ${GROUPNAME} CORE 0 CONFIG ${TRANSPORT} -ts 9530 -te 9560 -ma ${MULTICASTADDRESS} -mp ${MULTICASTPORT} > ${LOGS_DIR}/instance08.log 2>&1 &
./rungmsdemo.sh instance09 ${GROUPNAME} CORE 0 CONFIG ${TRANSPORT} -ts 9561 -te 9590 -ma ${MULTICASTADDRESS} -mp ${MULTICASTPORT} > ${LOGS_DIR}/instance09.log 2>&1 &
./rungmsdemo.sh instance10 ${GROUPNAME} CORE 0 CONFIG ${TRANSPORT} -ts 9631 -te 9690 -ma ${MULTICASTADDRESS} -mp ${MULTICASTPORT} > ${LOGS_DIR}/instance10.log 2>&1 &

echo "Waiting for group [${GROUPNAME}] to complete startup"
./gms_admin.sh waits ${GROUPNAME} -ma ${MULTICASTADDRESS} -mp ${MULTICASTPORT}

echo "Group startup has completed, shutting down group [${GROUPNAME}]"
./gms_admin.sh stopc ${GROUPNAME} -ma ${MULTICASTADDRESS} -mp ${MULTICASTPORT}
